// APDU plumbing shared by every Ledger transport.
//
// The protocol code (`official`, `official_sign`, `fvk`, `builder`) only
// depends on the `Device` trait and the APDU types, which compile on every
// target. The USB HID transport below is desktop-only (hidapi) and lives
// behind the `ledger` feature; mobile builds reach the device through
// `dart_device`, where the Flutter side owns the BLE/USB connection.
...
#[cfg(feature = "ledger")]
mod hid {
    use super::{APDUAnswer, APDUCommand, Device};
